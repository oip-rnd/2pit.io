...
class AccountSource
{
	...
	public static function getDescription($type)
	{
		...
		$description['properties'] = AccountSource::getConfig($type);
		$description['search'] = AccountSource::getConfigSearch($type, $description['properties']);
		$description['list'] = AccountSource::getConfigList($type, $description['properties']);
		$description['update'] = AccountSource::getConfigUpdate($type, $description['properties']);
		$description['groupUpdate'] = AccountSource::getConfigGroupUpdate($type, $description['properties']);
		$description['export'] = AccountSource::getConfigEXport($type, $description['properties']);
		return $description;
	}
    	...
    	if ($description) {
	    	foreach ($description['properties'] as $propertyId => $property) {
	    		if ($property['private'] && $data[$propertyId]) {
	    			$value = $context->getSecurityAgent()->unV2($data[$propertyId]);
    ...
    public static function getList($type, $params, $order = '+name', $limit = 50, $columns = null, $pageNumber = false, $itemCountPerPage = false)
    {
    	...
    	$context = Context::getCurrent();
    	if ($type) $description = AccountSource::getDescription($type);
    	else $description = null;
    	
    	$order = explode(',', $order);
    	foreach ($order as &$criterion) $criterion = substr($criterion, 1).' '.((substr($criterion, 0, 1) == '-') ? 'DESC' : 'ASC');
    	$select = AccountSource::getTable()->getSelect()
		...
		foreach ($params as $propertyId => $value) {
			...
			$property = AccountSource::getConfig($type)[$propertyKey];
			$entity = AccountSource::$model['properties'][$propertyKey]['entity'];
			$column = AccountSource::$model['properties'][$propertyKey]['column'];
...
/*			if ($propertyId == 'status') $where->in('core_account.status', explode(',', $value));
    	...
    	$select->where($where);
		$cursor = AccountSource::getTable()->selectWith($select, ($pageNumber) ? new AccountSource : null);
		if ($pageNumber) {
    ...
    public function denormalize() {
    	...
    	if ($this->contact_2) {
    		$this->contact_2 = VcardSource::get($this->contact_2_id);
    	...
    	if ($this->contact_3) {
    		$this->contact_3 = VcardSource::get($this->contact_3_id);
    	...
    	if ($this->contact_4) {
    		$this->contact_4 = VcardSource::get($this->contact_4_id);
    	...
    	if ($this->contact_5) {
    		$this->contact_5 = VcardSource::get($this->contact_5_id);
    ...
    {
    	$account = AccountSource::getTable()->get($id, $column);
    	 
    	if (!$account) return null;
    	$description = AccountSource::getDescription($account->type);
    	...
    	if ($account->contact_1_id && !$account->contact_1) {
	    	$account->contact_1 = VcardSource::get($account->contact_1_id);
		    	
		    $userContact = UserContactSource::get($account->contact_1_id, 'vcard_id');
		    if ($userContact) {
		    	$account->userContact = $userContact;

		    	$user = UserSource::get($userContact->user_id);
		    	$account->user = $user;
		    }
		    if (!$account->user) $account->user = UserSource::instanciate();
		    $account->username = $account->user->username;
	    }
	    else $account->contact_1 = VcardSource::instanciate();
        	
    	$account->contact_2 = ($account->contact_2_id) ? VcardSource::get($account->contact_2_id) : null;
    	$account->contact_3 = ($account->contact_3_id) ? VcardSource::get($account->contact_3_id) : null;
    	$account->contact_4 = ($account->contact_4_id) ? VcardSource::get($account->contact_4_id) : null;
    	$account->contact_5 = ($account->contact_5_id) ? VcardSource::get($account->contact_5_id) : null;
		$account->denormalize();	    
	    $account->properties = $account->getProperties($account->type, $description);
    ...
    public static function instanciate($type = null)
    {
		...
		$account->audit = array();
		$account->contact_1 = VcardSource::instanciate();
	...
	public function loadData($type, $data)
	{
		...
		$configProperties = AccountSource::getConfig($type);
		...
		foreach ($data as $propertyId => $value) {
			if (!array_key_exists($propertyId, $configProperties)) $errors[$propertyId] = "The accounts of type $type does not manage the property $propertyId";
				...
				elseif (in_array($property['type'], ['time'])) {
			    	if ($value && !AccountSource::checktime($value)) $errors[$propertyId] = "$propertyId should be a valid time according to the format hh:mm:ss";
				}
				elseif (in_array($property['type'], ['datetime'])) {
			    	if ($value && (!checkdate(substr($value, 5, 2), substr($value, 8, 2), substr($value, 0, 4)) || !AccountSource::checktime(substr($value, 11, 8)))) $errors[$propertyId] = "$propertyId should be a valid date & time according to the format yyyy-mm-dd hh:mm:ss";
				}
    ...
    public function add()
    {
    	$context = Context::getCurrent();
    	$this->id = null;
    	AccountSource::getTable()->save($this);
    	if (!$this->identifier) {
    ...
    {
    	$context = Context::getCurrent();
    	$account = AccountSource::get($this->id);
    ...
    public static function processInteraction($data, $interaction)
    {
		...
		$account = AccountSource::instanciate($type);
		$rc = $account->loadAndAdd($data, AccountSource::getConfig($type));
		return $rc;
    }
...
/*	    	$img = new MimePart($context->getConfig('customisation/esi/send-message/logo')['content']);
    ...
    {
    	$context = Context::getCurrent();
    	$account = AccountSource::getTable()->get($this->id);
    
    	// Isolation check
    	if ($update_time && $account->update_time > $update_time) return 'Isolation';
/*    	$user = UserSource::get($this->contact_1->id, 'vcard_id');
...
/*    	foreach ($description['properties'] as $propertyId) {
    	...
    	$contact4Data = array();
    	$contact5Data = array();
    	$entities = AccountSource::$model['entities'];
    	$properties = AccountSource::$model['properties'];
    	...
    	$account = null;
//		if (array_key_exists('identifier', $accountData) && $accountData['identifier']) $account = AccountSource::get($accountData['identifier'], 'identifier');

    	$vcard = null;
    	if (array_key_exists('email', $vcardData) && $vcardData['email']) {
	    	$vcard = VcardSource::get($vcardData['email'], 'email');
    	}
    	if (!$vcard) $vcard = VcardSource::instanciate();
		if (!$account) $account = $vcard->id ? current(AccountSource::getList($type, ['place_id' => $place->id, 'contact_1_id' => $vcard->id])) : null;
    	if ($account) {
    	...
    	if ($contact2Data) {
			$account->contact_2 = VcardSource::instanciate();
	    	$rc = $account->contact_2->loadData($contact2Data);
	    	if ($rc != 'OK') return ['500', $rc];
    	}

    	if ($contact3Data) {
			$account->contact_3 = VcardSource::instanciate();
	    	$rc = $account->contact_3->loadData($contact3Data);
	    	if ($rc != 'OK') return ['500', $rc];
    	}

    	if ($contact4Data) {
    		$account->contact_4 = VcardSource::instanciate();
	    	$rc = $account->contact_4->loadData($contact4Data);
	    	if ($rc != 'OK') return ['500', $rc];
    	}

    	if ($contact5Data) {
    		$account->contact_5 = VcardSource::instanciate();
		...
		$account->denormalize();
		$account->properties = $account->getProperties($account->type, AccountSource::getDescription($type));
    ...
    public function loadAndUpdate($data, $config, $update_time = null)
    {
		...
		// Load the data
		$this->contact_1 = VcardSource::get($this->contact_1_id);
    	...
    	if ($contact2Data) {
    		if ($this->contact_2_id) $this->contact_2 = VcardSource::get($this->contact_2_id);
    		else $this->contact_2 = VcardSource::instanciate();
	    	$rc = $this->contact_2->loadData($contact2Data);
	    	if ($rc != 'OK') return ['500', $rc];
    	}
    	
    	if ($contact3Data) {
    		if ($this->contact_3_id) $this->contact_3 = VcardSource::get($this->contact_3_id);
    		else $this->contact_3 = VcardSource::instanciate();
	    	$rc = $this->contact_3->loadData($contact3Data);
	    	if ($rc != 'OK') return ['500', $rc];
    	}

    	if ($contact4Data) {
    		if ($this->contact_4_id) $this->contact_4 = VcardSource::get($this->contact_4_id);
    		else $this->contact_4 = VcardSource::instanciate();
	    	$rc = $this->contact_4->loadData($contact4Data);
	    	if ($rc != 'OK') return ['500', $rc];
    	}
    	
    	if ($contact5Data) {
    		if ($this->contact_5_id) $this->contact_5 = VcardSource::get($this->contact_5_id);
    		else $this->contact_5 = VcardSource::instanciate();
    ...
    {
    	if (!AccountSource::$table) {
    		$sm = Context::getCurrent()->getServiceManager();
    		AccountSource::$table = $sm->get(AccountSourceTable::class);
    	}
    	return AccountSource::$table;
    }
}