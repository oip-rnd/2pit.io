...
class Community
{
    ...
    /** @var string */ public $authentication_token;
	/** @var int */ public $place_id;
    /** @var string */ public $identifier;
	/** @var string */ public $specifications;
	/** @var string */ public $name;
	/** @var array */ public $home_title;
    /** @var array */ public $home_description;
    /** @var array */ public $external_links;
    ...
    // Static fields
    /** @var \Model\GenericTable */ private static $table;
    ...
    public function exchangeArray($data)
    {
        ...
        $this->home_title = (isset($data['home_title'])) ? json_decode($data['home_title'], true) : null;
        $this->home_description = (isset($data['home_description'])) ? json_decode($data['home_description'], true) : null;
        $this->external_links = (isset($data['external_links'])) ? json_decode($data['external_links'], true) : null;
    ...
    public function getProperties()
    {
    	...
    	$data['home_title'] =  $this->home_title;
    	$data['home_description'] =  $this->home_description;
    ...
    public function toArray()
    {
    	...
    	$data['home_title'] = json_encode($this->home_title);
    	$data['home_description'] = json_encode($this->home_description);
    	$data['external_links'] = json_encode($this->external_links);
    	return $data;
    }
    ...
    public static function instanciate()
    {
    	...
    	$community->specifications = array();
    	$community->home_title = array();
    	$community->home_description = array();
    	$community->external_links = array();
    	return $community;
    }
    ...
    public function loadData($data) 
    {
    	...
    	if (array_key_exists('authentication_token', $data)) {
    		...
    		if (strlen($authentication_token) > 255) return 'Integrity';
    		// The authentication token is not audited
    		if ($this->authentication_token != $authentication_token) $this->authentication_token = $authentication_token;
    	}
		...
		if (array_key_exists('home_title', $data)) {
			$home_title = $data['home_title'];
			if ($this->home_title != $home_title) $auditRow['home_title'] = $this->home_title = $home_title;
		}

		if (array_key_exists('home_description', $data)) {
			$home_description = $data['home_description'];
			if ($this->home_description != $home_description) $auditRow['home_description'] = $this->home_description = $home_description;
		}

		if (array_key_exists('external_links', $data)) {
			$external_links = $data['external_links'];
			if ($this->external_links != $external_links) $auditRow['external_links'] = $this->external_links = $external_links;
		}
...
/*    	$rootDoc->acl = array('contacts' => array(), 'communities' => array($this->id => 'write'));
    ...
    public static function consumeCredits($live, $mailTo)
    {
    	...
    	foreach ($cursor as $instance) {
    		$unlimitedCredits = (array_key_exists('credit', $instance->specifications)) ? $instance->specifications['credit']['unlimitedCredits'] : false;
    ...
    {
    	$context = Context::getCurrent();
    	$community = Community::getTable()->get($this->id);
    	if ($update_time && $community->update_time > $update_time) return 'Isolation';
    	$this->status = 'deleted';
    	Community::getTable()->save($this);
    	return 'OK';
    ...
    public static function getTable()
    {
    	if (!Community::$table) {
    		...
    		Community::$table = $sm->get(CommunityTable::class);
    	}
