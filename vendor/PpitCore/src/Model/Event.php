...
class Event
{
	...
	public static function getConfigProperties($type) {
		...
		if (in_array('matched_accounts', $description['properties']) || in_array('rewards', $description['properties'])) {
			$accounts = Account::getList($context->getConfig('event/' . $type . '/property/matched_accounts')['account_type'], ['status' => 'new,interested,candidate,answer,conversion,committed,visa,active,retention'], '+name', null, ['id', 'n_fn', 'email', 'name']);
		}
...
/*			$perimeters = $context->getPerimeters();
    ...
    // Static fields
    /** @var \Model\Event */ private static $table;
...
//	    				$connection->rollback();
    ...
    public static function getTable()
    {
    	if (!Event::$table) {
    		...
    		Event::$table = $sm->get(EventTable::class);
    	}
    	return Event::$table;
    }
}
