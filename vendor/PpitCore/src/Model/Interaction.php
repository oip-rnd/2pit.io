...
class Interaction
{
    ...
    // Static fields
    /** @var \Model\Interaction */ private static $table;
	...
	public function loadData($data)
	{
		...
		if (array_key_exists('route', $data)) {
	    	$route = trim(strip_tags($data['route']));
			if (strlen($route) > 255) return 'Integrity';
			if ($this->route != $route) $auditRow['route'] = $this->route = $route;
		}
    ...
    public function processInput()
    {
    	...
    	if ($this->format == 'application/json') {
    		...
    		$globalRc = call_user_func($function, $content, $this);
    	...
    	elseif ($this->format == 'text/csv') {
    		...
    		foreach($rows as $row) {
    			if ($i == 0) {
	   				$firstRow = str_getcsv($row, ";");
	   				$identifiers = array();
	   				foreach ($firstRow as $column) $identifiers[] = $column;
    			}
    			elseif ($i >= $context->getConfig('event/csv/'.$this->category)['nb_lines_to_ignore']) {
    				$array = str_getcsv($row, ";");
    				if (count($array) != count($firstRow)) {
    					$row .= ';"error: bad number of columns"';
    				}
    				else {
    					$content = array();
    					for ($i = 0; $i < min(count($firstRow), count($identifiers)); $i++) {
    						$content[$identifiers[$i]] = $array[$i];
    					}
    					$function = $context->getConfig('interaction/type/'.$this->type)['processor'];
    					$rc = call_user_func($function, $content, $this);
    					$row .= '"'.$rc.'"';
    				}
    			}
    			$i++;
...
//    		$data['content'] = $newContent;
			...
			foreach ($content as &$row) {
				...
				try {
					$rc = call_user_func($function, $row, $this);
					$connection->commit();
					$row['result'] = $rc;
					if ($rc[0] == '200') $row['identifier'] = $rc[1];
					else $data['http_status'] = $rc[0];
				}
	    	   	catch (\Exception $e) {
	    			$connection->rollback();
	    	    }
