...
/**
 * The VcardSource class manages the data related to a contact.
 * A VcardSource stores for a given contact:
...
class VcardSource
{
    ...
    // Static fields
    /** @var \Model\VcardSource */ private static $table;
    ...
    /**
     * Returns an array of VcardSource instances filtered on the optional given community (no filtering on community if not provided):
	...
	public static function getList($community_id, $params, $major = 'n_last', $dir = 'ASC', $mode = '')
    {
    	...
    	$select = VcardSource::getTable()->getSelect();
    	...
    	$cursor = VcardSource::getTable()->selectWith($select);
    ...
    {
    	$context = Context::getCurrent();
    	$vcard = VcardSource::getTable()->get($id, $column);
    	if ($vcard) {
...
/*	    	$community_id = $vcard->community_id;
    ...
    public function loadData($data)
    {
    	...
    	if (array_key_exists('email', $data)) {
    		...
    		if ($email && !preg_match(VcardSource::$emailRegex, $email)) return 'Integrity';
...
//	    	if ($tel_work && !preg_match(VcardSource::$telRegex, $tel_work)) return 'Integrity';
...
//	    	if ($tel_cell && !preg_match(VcardSource::$telRegex, $tel_cell)) return 'Integrity';
    ...
    public function add()
    {
    	$context = Context::getCurrent();
    	$this->status = 'new';
       	VcardSource::getTable()->save($this);
    	return 'OK';
    }
    ...
    public function update($update_time)
    {
    	...
    	// Check isolation
		$vcard = VcardSource::getTable()->get($this->id);
    ...
    public function isUsed($object)
    {
    	...
    	if (get_class($object) == 'Model\Instance') {
    		if (Generic::getTable()->cardinality('core_vcard_source', array('instance_id' => $object->id)) > 0) return true;
    	}
    	// Allow or not deleting an community
    	if (get_class($object) == 'Model\Community') {
    		if (Generic::getTable()->cardinality('core_vcard_source', array('community_id' => $object->id)) > 0) return true;
    	}
    	return false;
    }
    ...
    {
    	if (Generic::getTable()->cardinality('core_event_source', array('status != ?' => 'deleted', 'place_id' => $this->id)) > 0) return false;
    ...
    public function delete($update_time)
    {
    	...
    	$vcard = VcardSource::getTable()->get($this->id);
    	if ($update_time && $vcard->update_time != $update_time) return 'Isolation';
    	VcardSource::getTable()->delete($this->id);
    	return 'OK';
    }
    ...
    {
    	if (!VcardSource::$table) {
    		$sm = Context::getCurrent()->getServiceManager();
    		VcardSource::$table = $sm->get('PpitCore\Model\VcardSourceTable');
    	}
    	return VcardSource::$table;
    }
}