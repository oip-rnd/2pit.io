...
class UserSource extends Context
{
	public $user_id;
    public $instance_id;
    public $username;
    ...
    { 
    	$select = UserContactSource::getTable()->getSelect()->where(array('user_id' => $this->getUserId()));
    	$cursor = UserContactSource::getTable()->transSelectWith($select);
    	$contacts = UserContactSource::getList();
 //   	foreach ($cursor as $userContact) $contacts[$userContact->vcard_id] = $userContact;
    ...
    public static function getList($instance_id, $app, $params, $major = 'n_fn', $dir = 'ASC', $mode = 'todo')
    {
    	...
    	if (!$dir) $dir = 'ASC';
    	$select = UserSource::getTable()->getSelect()
...
//    		$where->notEqualTo('state', 1);
    	...
    	$select->where($where);
    	$user_cursor = UserSource::getTable()->selectWith($select);

		$myPerimeter = VcardSource::get($context->getContactId())->perimeters;
		$places = array();
    ...
    public static function get($id, $column = 'user_id')
    {
    	...
    	$user = UserSource::getTable()->transGet($id, $column);
    	if ($user) {
    		$userContact = UserContactSource::get($user->user_id, 'user_id');
            if ($userContact) $user->contact = VcardSource::get($userContact->vcard_id);
            else $user->contact = VcardSource::getTable()->transGet($user->vcard_id);
    	}
		return $user;
    }
    ...
    public function loadData($request, $contact, $instance_id = null)
    {
    	...
    	$user = UserSource::getTable()->transGet($this->username, 'username');
    ...
    public function add($email = false, $notify = false)
    {
		...
		if (UserSource::getTable()->transGet($this->username, 'username')) return 'Duplicate';
		$user_id = UserSource::getTable()->save($this);
/*	
		if ($notify) {
    ...
    public function update($update_time)
    {
    	...
    	if ($update_time) {
			$user = UserSource::getTable()->transGet($this->user_id);
			if ($user->update_time != $update_time) return 'Isolation';
    	}
		UserSource::getTable()->save($this);
		return 'OK';
    }
    ...
    {
    	if (get_class($object) == 'Model\VcardSource') {
    ...
    public function delete($update_time)
    {
    	...
    	$vcard = VcardSource::getTable()->get($this->vcard_id);
    	...
    	UserSource::getTable()->delete($this->user_id);
    	UserContactSource::getTable()->multipleDelete(array('user_id' => $this->user_id));
    	return 'OK';
    }
    ...
    public static function getTable()
    {
    	if (!UserSource::$table) {
    		...
    		UserSource::$table = $sm->get('PpitCore\Model\UserSourceTable');
    	}
    	return UserSource::$table;
    }
}
