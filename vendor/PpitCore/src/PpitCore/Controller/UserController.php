...
use PpitCore\ViewHelper\SsmlUserViewHelper;
			    	...
			    	if ($rc != 'OK') $error = $rc;
						...
						if (!$error) {
							...
							if ($creationMode) {
								...
								$rc = $userContact->add();
								if ($rc == 'OK') $context->getSecurityAgent()->requestPasswordInit($user, true, $contact);
								else $error = $rc;
							}
							if ($rc == 'OK') {
				    			$connection->commit();
    ...
    public function lostPasswordAction()
    {
    	...
    	if ($request->isPost()) {
    		...
    		if ($csrfForm->isValid()) { // CSRF check
    			...
    			$context->getSecurityAgent()->requestPasswordInit($user, true);
       			$message = 'OK';
    		}
    	}
    	$view = new ViewModel(array(
	...
	public function v1Action()
	{
		...
		if ($this->request->isGet()) {
			...
			elseif ($request == 'request-activation') {
				...
				$link = $url('user/v1', ['id' => $user->user_id], ['force_canonical' => true]).'?account_id='.$account->id.'&request=activate'.'&hash='.$token;
			...
			if ($request == 'lost-password') {
		    	...
		    	$link = $url('landing/template2', [], ['force_canonical' => true]).'?panel=modalInitPasswordForm'.'&hash='.$token;
				$email_body = sprintf($email_body, $link);
