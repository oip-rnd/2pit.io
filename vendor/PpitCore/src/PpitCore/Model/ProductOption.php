...
class ProductOption implements InputFilterAwareInterface
{
    ...
    public static function getList($type, $params, $major = 'reference', $dir = 'ASC', $mode = 'search')
    {
    	...
    	$where = new Where();
    	$where->notEqualTo('core_product_option.status', 'deleted');
    	...
    	if ($mode == 'todo') {
    		...
    		foreach ($params as $propertyId => $property) {
    			if (substr($propertyId, 0, 4) == 'min_') $where->greaterThanOrEqualTo('core_product_option.'.substr($propertyId, 4), $params[$propertyId]);
    			elseif (substr($propertyId, 0, 4) == 'max_') $where->lessThanOrEqualTo('core_product_option.'.substr($propertyId, 4), $params[$propertyId]);
    			else $where->like('core_product_option.'.$propertyId, '%'.$params[$propertyId].'%');
    		}
    	}
...
/*    	$select = ProductOption::getTable()->getSelect()->where(array('reference' => $this->reference));
    ...
    public static function getTable()
    {
    	if (!ProductOption::$table) {
    		...
    		ProductOption::$table = $sm->get('PpitCore\Model\ProductOptionTable');
    	}
