...
class Event implements InputFilterAwareInterface
{
	...
	/** @var string */ public $begin_date;
	/** @var string */ public $end_date;
	/** @var int */ public $day_of_week;
	/** @var int */ public $day_of_month;
	/** @var string */ public $begin_time;
	/** @var string */ public $end_time;
	/** @var int */ public $time_zone;
	/** @var string */ public $location;
	/** @var float */ public $latitude;
	/** @var float */ public $longitude;
     ...
     * @return Event[]
     */
    public static function getList($type, $params, $major = 'caption', $dir = 'ASC', $mode = 'todo')
    {
    	$select = Event::getTable()->getSelect()
    		->join('core_community', 'core_event.community_id = core_community.id', array('community_name' => 'name'), 'left')
    		->join('core_place', 'core_community.place_id = core_place.id', array('place_identifier' => 'identifier', 'place_caption' => 'caption'), 'left')
			->order(array('core_event.begin_date ASC', 'core_event.begin_time ASC'))
    		->limit(200);
    		$where = new Where;
    	...
    	if ($mode == 'todo') {
//    		$where->lessThanOrEqualTo('core_event.begin_date', date('Y-m-d'));
//    		$where->greaterThanOrEqualTo('core_event.end_date', date('Y-m-d'));
    	}
    	else {
    		// Set the filters
    		foreach ($params as $propertyId => $property) {
    			if ($propertyId == 'community_name') $where->like('core_community.name', '%'.$params[$propertyId].'%');
    			elseif ($propertyId == 'place_identifier') $where->like('core_place.identifier', '%'.$params[$propertyId].'%');
    			elseif (substr($propertyId, 0, 4) == 'min_') $where->greaterThanOrEqualTo('core_event.'.substr($propertyId, 4), $params[$propertyId]);
    			elseif (substr($propertyId, 0, 4) == 'max_') $where->lessThanOrEqualTo('core_event.'.substr($propertyId, 4), $params[$propertyId]);
    			else $where->like('core_event.'.$propertyId, '%'.$params[$propertyId].'%');
    		}
    	}
    	$select->where($where);
    ...
    /**
     * Retrieve from the database the event of a given time and for a given community that occur in the future.
     * @param string $type
     * @param string $community_id
     * @return Event[]
     */
    public static function retrieveComing($type, $community_id)
    {
    ...
    public static function get($id, $column = 'id')
    {
    	...
    	$community = Community::get($event->community_id);
    	if ($community) $event->community_name = $community->name;
