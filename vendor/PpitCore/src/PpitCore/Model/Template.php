...
/**
 * The Template class hosts the specification of page templates.
 *
 * A template has an identifier and a specification.
 */
class Template implements InputFilterAwareInterface
{
     ...
     * If $mode == 'todo', the list is filtered on only active templates.
     ...
     * Deleted templates remains in the database with a 'deleted' status, so in any case, the getList methods filters templates with the 'deleted' status. 
    ...
    {
    	$select = Template::getTable()->getSelect()
    		->order(array($major.' '.$dir, 'identifier'));
    	$where = new Where;
    	$where->notEqualTo('core_template.status', 'deleted');

    	// Todo list vs search modes
    	if ($mode == 'todo') {
    		$where->equalTo('core_template.status', 'active');
    	}
    	else {
    		// Set the filters
    		foreach ($params as $propertyId => $property) {
    			if (substr($propertyId, 0, 4) == 'min_') $where->greaterThanOrEqualTo('core_template.'.substr($propertyId, 4), $params[$propertyId]);
    			elseif (substr($propertyId, 0, 4) == 'max_') $where->lessThanOrEqualTo('core_template.'.substr($propertyId, 4), $params[$propertyId]);
    			else $where->like('core_template.'.$propertyId, '%'.$params[$propertyId].'%');
    		}
    	}
    	$select->where($where);

    	$cursor = Template::getTable()->selectWith($select);
    	$templates = array();
    	foreach ($cursor as $template) {
    		$template->properties = $template->getProperties();
    ...
    /**
     * Retrieve from the database the template having the giving value as the given specified column ('id' as a default).
     * @param string $id
     * @param string $column
     * @return Place
     */
    public static function get($id, $column = 'id')
    {
    	...
    	$template = Template::getTable()->get($id, $column);
    ...
    /**
     * Loads the data into the Template object depending of an array, typically constructed by the controller with value extracted from an HTTP request.
    ...
    /**
     * Adds a new row in the database after checking that it does not conflict with an existing, not deleted, template with the same 'identifier'. 
     * In such a case the methods does not affect the database and returns 'Duplicate', otherwise it returns 'OK'.
     * @return string
     */
	public function add()
    {
		if (Generic::getTable()->cardinality('core_template', array('status != ?' => 'deleted', 'identifier' => $this->identifier)) > 0) return 'Duplicate';
    	$this->id = null;
    	Template::getTable()->save($this);
		return ('OK');
    }
    ...
    {
    	$template = Template::get($this->id);
    	if ($update_time && $template->update_time > $update_time) return 'Isolation';
    	Template::getTable()->save($this);
		return ('OK');
    }
    
    /**
     * Checks if this template can de deleted. 
     * @return boolean
     */
    ...
    public static function getTable()
    {
    	if (!Template::$table) {
    		...
    		Template::$table = $sm->get('PpitCore\Model\TemplateTable');
    	}
