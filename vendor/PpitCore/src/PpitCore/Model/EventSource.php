...
/**
  * EventSource is the class supporting generic interactions between applications
  *
...
class EventSource
{
	...
	public static function getConfigProperties($type) {
		...
		foreach($description['properties'] as $propertyId) {
			...
			elseif ($propertyId == 'account_id') {
				$property['modalities'] = array();
				foreach (AccountSource::getList($property['account_type'], [], '+name', null) as $account) $property['modalities'][$account->id] = ['default' => $account->n_fn.' - '.$account->email];
			}
	...
	public static function getDescription($type)
	{
		...
		$description['properties'] = EventSource::getConfigProperties($type);
		$description['search'] = EventSource::getConfigSearch($type, $description['properties']);
		$description['list'] = EventSource::getConfigList($type, $description['properties']);
		$description['update'] = EventSource::getConfigUpdate($type, $description['properties']);
		$description['export'] = EventSource::getConfigEXport($type, $description['properties']);
		return $description;
	}
    ...
    // Static fields
    /** @var \Model\EventSource */ private static $table;
    ...
    /**
     * Returns an array of EventSource instances indexed by the event primary key in the database :
     ...
     * - For array based algorythms needs, a $property array is provided using getProperties() on EventSource properties.
     * @param $params[]
     * @param string $major
     * @param string $dir
     * @param string $mode
    ...
    public static function getList($type, $params, $order = '-update_time', $limit = 200, $columns = null, $mode = 'search', $dimensions = array())
    {
    	...
    	$select = EventSource::getTable()->getSelect()
...
//	    	$where->equalTo('core_event.status', 'new');
    	...
    	$cursor = EventSource::getTable()->selectWith($select);
    	$events = array();
    	$i = 0;
	...
	public static function getListV2($description, $params, $order = '-update_time', $limit = 200, $columns = null)
	{
		...
		foreach ($params as $propertyId => $predicate) {
			...
			$entity = EventSource::$model['properties'][$propertyId]['entity'];
			$column = EventSource::$model['properties'][$propertyId]['column'];
    ...
    {
		$select = EventSource::getTable()->getSelect()->order(array('begin_date ASC', 'begin_time ASC'));
    	$where = new Where;
    ...
    {
    	$context = Context::getCurrent();
    	$event = EventSource::getTable()->get($id, $column, $id2, $column2, $id3, $column3, $id4, $column4);
    	...
    	if ($event) {
	    	...
	    	$account = AccountSource::get($event->account_id);
	    	if ($account) {
	    		$vcard = VcardSource::get($account->contact_1_id);
	    		if ($vcard) {
		    	...
		    	$vcard = VcardSource::get($event->vcard_id);
		    	if ($vcard) {
		    		$event->n_fn = $vcard->n_fn;
		    		$event->n_first = $vcard->n_first;
		    		$event->n_last = $vcard->n_last;
    ...
    public function join()
    {
    	...
    	foreach ($dimensions as $dimensionId => $dimension) {
    		$rows = EventSource::getList($dimension['type'], array($dimension['dimension_key'] => $this->properties[$dimension['event_key']]), '+id', null, null, 'search', array());
    ...
    /**
     * Loads the data into the EventSource object depending of an array, typically constructed by the controller with value extracted from an HTTP request.
    ...
    {
    	$this->id = null;
    	EventSource::getTable()->save($this);
		return ('OK');
    }
    ...
    public static function processInteraction($data, $interaction)
    {
    	...
    	// Normalize the data
		$properties = EventSource::getConfigProperties($type);
...
/*    			elseif ($property['type'] == 'date' && $value) {
    	...
    	$event = EventSource::instanciate($type);
    	...
    	if ($interaction->format == 'text/csv') {
    		...
    		foreach ($context->getConfig('event/csv/'.$interaction->category)['targets'] as $targetId => $target) {
		    	...
		    	foreach($target as $propertyId => $expression) {
		    		...
		    		elseif ($propertyId == 'n_fn') {
		    			$vcard = VcardSource::get($value, 'n_fn');
	...
	/*    		foreach ($context->getConfig('event/type')['joinKeys'] as $joinKeyId => $joinKey) {
    	...
    	elseif ($interaction->format == 'application/json') {
	    	if ($data['action'] == 'update' || $data['action'] == 'delete') $event = EventSource::get($data['identifier'], 'identifier');
	    	elseif ($data['action'] == 'add') $event = EventSource::instanciate();
	    	...
	    	elseif ($data['action'] == 'add' || $data['action'] == 'update') {
	    	    ...
	    	    if (array_key_exists('n_fn', $data)) {
	    			$vcard = VcardSource::get($data['n_fn'], 'n_fn');
...
//	    			$connection->rollback();
	    	...
	    	elseif ($data['action'] == 'synchronize') {
	    		...
	    		foreach ($data['rows'] as $row) {
	    			...
	    			$old = EventSource::get($event->identifier, 'identifier');
	    			$rc = $old->delete(null);
	    		    if ($rc != 'OK') {
//	    				$connection->rollback();
			    		return $rc;
...
//	    				$connection->rollback();
    ...
    /**
     * Returns the object to relational manager for the EventSource class
     */
    public static function getTable()
    {
    	if (!EventSource::$table) {
    		$sm = Context::getCurrent()->getServiceManager();
    		EventSource::$table = $sm->get('PpitCore\Model\EventSourceTable');
    	}
    	return EventSource::$table;
    }
}
