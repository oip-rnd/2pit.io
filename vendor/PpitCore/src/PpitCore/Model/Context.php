...
class Context implements InputFilterAwareInterface
{
	...
	/**
	 * Returns the id (primary key) of the current instance (\PpitCore\Model\Instance).
	 * @return int
	 */
    ...
    /**
     * Returns the user_id (primary key on \PpitCore\Model\User) of the current connected user. Returns null in the case of a guest session
     * @return int|NULL
     */
    public function getUserId() { return Context::$static_user_id; }
    
    /**
     * Returns the contact list (array of \PpitCore\Model\Vcard) for the current connected user. Returns an empty array in the case of a guest session
     * @return \PpitCore\Model\Vcard[]
     */
    ...
    /**
     * Returns the formated Name for the current contact (\PpitCore\Model\Vcard)
     * @return string|NULL
     */
   	public function getFormatedName() { return Context::$static_formated_name; }
   	
   	/**
   	 * Returns the id (primary key on \PpitCore\Model\Community) of the current contact's community
   	 * @return int|NULL
   	 */
    public function getCommunityId() { return Context::$static_community_id; }
    
    /**
     * Returns the current id (primary key on \PpitCore\Model\Vcard) of the contact associated to the current connected user
     * @return int|NULL
     */
    public function getContactId() { return Context::$static_vcard_id; }
    
    /**
     * Returns the id (primary key on \Ppitcore\Model\Place) of the place for the current connected user
     * @return int|NULL
     */
    ...
    /**
     * Returns the deciaml separator corresponding to the current locale.
     * Returns '.' for US 
