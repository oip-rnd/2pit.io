    	...
    	if ($mode == 'todo') {
    		...
    		foreach ($params as $propertyId => $value) {
				...
				elseif (substr($propertyId, 0, 8) == 'account_') {
					if (!$value) $where->isNull('core_account.'.substr($propertyId, 8));
					elseif (strpos($value, ',')) $where->in('core_account.'.substr($propertyId, 8), array_map('trim', explode(',', $value)));
    ...
    public function loadData($data, $files = array()) {
    	...
    	$context = Context::getCurrent();
    		...
    		if (array_key_exists('bank_name', $data)) {
				$this->bank_name = trim(strip_tags($data['bank_name']));
				if (strlen($this->bank_name) > 255) return 'Integrity';
			}
    		if (array_key_exists('invoice_n_last', $data)) {
				$this->invoice_n_last = trim(strip_tags($data['invoice_n_last']));
				if (strlen($this->invoice_n_last) > 255) return 'Integrity';
			}
