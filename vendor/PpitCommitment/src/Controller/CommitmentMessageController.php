...
//				$message->loadDataFromRequest($request);
    ...
    public function downloadInvoiceAction()
    {
    	...
    	$content = $pdf->Output('invoice-'.$context->getInstance()->caption.'-'.$invoice['identifier'].'.pdf', 'D');
    ...
    public function guestDownloadInvoiceAction()
    {
    	...
    	// Temporary
    	if ($this->request->getUri()->getHost() == 'www.p-pit.fr') {
    		return $this->redirect()->toUrl('https://mantesalo.p-pit.fr/commitment-message/guest-download-invoice/'.$id.'?hash='.$token);
    	}
