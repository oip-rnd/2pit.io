...
{    
	...
	public function requestPasswordInit($user, $sendMail, $contact = null) {
		$context = Context::getCurrent();
		...
		if ($sendMail) {
			...
			$link = $context->getConfig()['ppitCoreSettings']['domainName'].$url('user/initpassword', array('id' => $user->user_id)).'?hash='.$user->password_init_token;
